...
def test_has_waveform():
    for i in (4, 5, 9, 10):
        assert pylas.point.dims.format_has_waveform_packet(i), "Point format {} should have waveform".format(i)

    for i in (0, 1, 2, 3, 6, 7, 8):
        assert not pylas.point.dims.format_has_waveform_packet(i), "Point format {} should not have waveform".format(i)
